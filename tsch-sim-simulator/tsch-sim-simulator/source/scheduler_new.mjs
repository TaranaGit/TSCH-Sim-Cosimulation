...
function set_timings()
{
    ...
    // after conversion "slot_timings" in 'time.mjs' will be be 0.01 with defined slot_frame_size; i.e if slot-size = 5 we have 0.01,0.01,0.01,0.01,0.01 
...
function read_schedule(node) {
    ...
    try {
        ...
        const schedule_struct = config.SCHEDULE
...
{
    log.log(log.INFO, node, "TSCH", `On new time source called from New Scheduler [node_id becomes root] [SCHEDULER NEW]`);
}
...
export function initialize()
{
    ...
    const default_config = {
        ...
        TSCH_SCHEDULE_CONF_DEFAULT_LENGTH: 13
    };
